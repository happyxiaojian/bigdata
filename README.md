...
**redis**中的有序集合是由跳表来完成的, 不是用的红黑树

**心得**: 工作中编程的边界问题比逻辑问题同样重要, 逻辑问题完成初始的业务, 边界问题考虑不好的话后期上线会出现很多问题.

提高编程速度的方法，其实可以参考高考做数学题的方法，为了提高解题的速度，除了大量的刷题扩大自己知识面之外，另外一个就是把常见的解题的套路印在自己的脑子当中，提高提取的速度。相当于缓存和搜索引擎的区别。



### #2019年4月12日 10:58:36

订购了许式伟在极客时间上的专栏 < 许式伟的架构课 > 来自书里面的摘要:

> 当我们把程序员类比成建筑师时，按照能力水平来分，我觉得大体可以分为三个层次:搬砖师, 工程师, 架构师

以我自己的对自己的了解可以知道的是, 我现在的这个只能做个搬砖师. 在公司做的也是这方面的工作. 如果我想进入到更深的阶层的话, 必须向工程师的要求去要求自己

1. **工程师**必须熟练的敲出常用的数据结构和算法, 做到手写无误的程度, 要求的标准是无障碍的敲出**红黑树**的代码
2. **工程师**必须牢记常用的设计模式, 象这些思想性质的知识必须以记忆的形式储存在脑子里面, 才能做到信手拈来
3. **工程师**对数据库的理解应该要达到调优的层面了, 不能还局限于普通的对表的操作
4. **工程师**做的工作应该是系统级别的了, 不应该是语言逻辑方面的问题了. 深入的了解一门语言做到对任何的细枝末节都心中有数的程度, 再也不是仅仅理解的程度, 记忆就像是把知识存储在内存里面可以快速的调用, 仅仅是理解就像是靠搜索引擎去查取想用的知识一样,效率可想而知, 工资 = 新做的成果 * 效率



没次看服务端文章的时候都逃不开几个名词:

多进程, 协程, 进程间通信, 消息队列, 网络, 锁 

### #2019年4月10日 12:26:16

redis 是单进程的, 所以为了充分的使用cpu的资源, 一般会使用多个redis进程, 来同时工作

1)看视频快速了解 --> (这个节段的主要的任务是熟悉路径, 千万不能太慢)

2)把别人代码运行一遍 --> (一定要运行一遍, 别想着自己把它写出来)

3)在原来代码的基础上改代码 --> (为自己写代码做准备)

4)把代码敲7遍 --> (为自己写代码做准备)

5)看书更深入的去理解原理. 

只所以给自己制定出这样的流程是因为自己学东西的时候对自己要求太高了, 这样往往却并没有写多少的代码, 总是在看的阶段, 但编程是一个手艺活, 关键在与写, 实践更重要. 做中学

### #2019年4月8日 15:32:18

人的成就不论大小都需要经过努力和苦难才能达成。--郝林

golang 语言中 什么时候使用make 和 new ?

对于**引用类型**的变量，我们不光要声明它，还要为它**分配**内容空间，否则我们的值放在哪里去呢？分配内存空间就要使用make和new

make也是用于内存分配的，但是和new不同，它只用于chan、map以及切片的内存创建，而且它返回的类型就是这三个类型本身，而不是他们的指针类型，因为这三种类型就是引用类型，所以就没有必要返回他们的指针了

### #2019年4月3日 11:21:50

昨天到凌晨2点才回去, 把昨天的任务完成了, 但是把时间的部分跳过去了, 无脑看视频的话速度快忘的也快, 看着别人都找到了工作, 真不知道他们是怎么学的.

环境的运行情况有出现了问题, Hadoop在启动后一直处于safemode模式, 而且退出不了, 到现在也没有好的解决方案.

昨天充了百度云盘的会员, 把之前下了快一个月都没有下完的课程一晚上就下完了. 花钱买时间和注意力, 之前好傻一直在哪里傻傻的免费的下载, 浪费了很多的精力. 

预估大数据的学习周期要到9月份了, 要少赚好多钱, 自学的速度好慢. 
...
- [x] 2 生态架构课--05 Hive      4h    ----> 0%
- [x] 06 Flume 1h 日志收集系统 -----> 100%
